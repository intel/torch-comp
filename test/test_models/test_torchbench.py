...
if mode is None:
    mode = "all"
assert type(mode) is str and mode.lower() in [
    "train",
    "inf",
    "all",
], "please specify the TEST_TORCHBENCH_MODE to be train/inf/all"
bench_file = "run.py"
...
models_list_specific_env_flag = {
    ...
    "vision_maskrcnn": {
        "set": {"CCL_ZE_IPC_EXCHANGE": "sockets"},
...
def set_the_running_mode(model):
    if mode == "train":
        ...
        raise RuntimeError(
            "[error] no mode is specified, please check env flag TEST_TORCHBENCH_MODE"
        )
...
def generate_required_env(model):
    ...
    if model in models_list_specific_env_flag:
        ...
        assert (
            "set" in required_env and "unset" in required_env
        ), "check the models_list_specific_env_flag, it is wrongly set"
...
def testing(model, test_mode, count):
    ...
    try:
        ...
        if returncode != 0:
            ...
            is_not_supported = all(
                [key.lower() in stderr_msg.lower() for key in unsupported_str_list]
            )
...
for idx, model in enumerate(results_summary.keys()):
    assert (
        model in results_summary
    ), "Fatal error, the model is not in the testing records"
    ...
    for test_mode in ["train", "inf"]:
        ...
        if result_key in results_summary[model]:
            if "pass" in results_summary[model][result_key]:
                ...
                raise RuntimeError(
                    "model {} is not recorded into the results, check if it is running".format(  # noqa:E501
                        model
                    )
                )
    ...
    for test_mode in ["train", "inf"]:
        ...
        if result_key in results_summary[model]:
            if "pass" in results_summary[model][result_key]:
                ...
                raise RuntimeError(
                    "model {} is not recorded with the results, check if it run or not".format(  # noqa:E501
                        model
                    )
                )
